...
An agent is where an app and a model meet the world. Nobody turning an agent loose
worries about parameter counts. They worry: can it read my filesystem, can it run
shell commands, where does my data go, what does it do without a human in the loop,
and what does it phone home? No existing format answers that crisply.
`AGENT_FACTS.md` does.
...
tools:
  ...
  toolsets: []             # URLs/paths to TOOL_FACTS.md files (the itemized detail)
reach:
...
1. **Dogfood first:** the first real `AGENT_FACTS.md` labels a reference agent
   ...
   follow.
2. **Directory at launch:** label popular public agents (CLI agents, IDE
   extensions, autonomous services, chatbots), human-review judgment fields, and
   launch agentfacts.dev as a "what can this agent actually touch" comparison
   directory on day one. The shared MCP-ecosystem crawl (vision doc) feeds the
   toolset rollups here while its per-tool detail feeds toolfacts.dev. That is
   the SEO surface and the HN story — especially the `undisclosed` and `reach`
   columns.
...
- A stranger can read an `AGENT_FACTS.md` and answer, in under a minute: can it
  touch my files, can it hit the network, what runs without approval, and what
  leaves my machine.
